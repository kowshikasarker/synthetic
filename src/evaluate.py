...
def parse_args():
    ...
    parser.add_argument("--test_feature_path", type=str,
                        help="test features in .tsv format after preprocessing, the first column name should be 'Sample' containing sample identifiers and the rest of the columns should contain metabolomic concentrations",
                        required=True, default=None)
    parser.add_argument("--test_condition_path", type=str,
                        help="test condiiton in .tsv format after preprocessing, the first column name should be 'Sample' containing sample identifiers, and the rest of the columns should each denote one disease group and contain either 0 or 1",
                        required=True, default=None)
    
    parser.add_argument("--syn_feature_path", type=str,
                        help="path to the synthetic micriobiome data in .tsv format, the first column name should be 'Sample' containing sample identifiers and the rest of the columns should contain metabolomic concentrations",
                        required=True, default=None)
    parser.add_argument("--syn_condition_path", type=str,
                        help="path to the synthetic metabolome data in .tsv format, the first column name should be 'Sample' containing sample identifiers, and the rest of the columns should each denote one disease group and contain either 0 or 1",
                        required=True, default=None)
...
def evaluate_mean_std_corr(exp_df, syn_df, out_dir):
    ...
    df = pd.concat([exp_mean, syn_mean, exp_std, syn_std], axis=1, keys = ['exp_mean', 'syn_mean', 'exp_std', 'syn_std'])
    print('df', df.shape)
...
def evaluate_classifier(model_name, train_x, test_x, train_y, test_y):
    ...
    print(model_name, 'time', end_time-start_time)
...
def evaluate_classification_utility(exp_train_x, exp_train_y,
    ...
    df = pd.DataFrame.from_records(records)
    df.to_csv(out_dir + '/classification_utility.tsv', sep='\t', index=False)
