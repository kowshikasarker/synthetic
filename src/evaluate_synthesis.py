...
def evaluate_augmentation(train_df,
    ...
    train_meta = train_meta.rename(columns=col_map) 
    test_meta = test_meta.rename(columns=col_map) 
    syn_meta = syn_meta.rename(columns=col_map) 
    
    train_meta = train_meta.idxmax(axis=1)
    test_meta = test_meta.idxmax(axis=1)
    syn_meta = syn_meta.idxmax(axis=1)
    
    meta_col = 'Cohort'
    train_meta.name = meta_col
    test_meta.name = meta_col
    syn_meta.name = meta_col
