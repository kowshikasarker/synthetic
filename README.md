...
```
  ...
  --corr_top_pct CORR_TOP_PCT
                        top percentage of correlation edges to use, for every metabolite this percentage of top correlated metabolites are connected with correlation edges
  --prior_path PRIOR_PATH
                        path to prior edges in .tsv format should contain three columns, the first two containing metabolites and the third column containing the weight of the prior connection
  --prior_top_pct PRIOR_TOP_PCT
                        top percentage of prior edges to use, for every metabolite this percentage of top prior edges is kept based on higher weights
...
```
  --train_feature_path TRAIN_FEATURE_PATH
                        train features in .tsv format after preprocessing, the first column name should be 'Sample' containing sample identifiers and the rest of the columns should contain metabolomic concentrations
  --train_condition_path TRAIN_CONDITION_PATH
                        train condition in .tsv format after preprocessing, the first column name should be 'Sample' containing sample identifiers, and the rest of the columns should each denote one disease group and contain
                        either 0 or 1
  --val_feature_path VAL_FEATURE_PATH
                        validation features in .tsv format after preprocessing, the first column name should be 'Sample' containing sample identifiers and the rest of the columns should contain metabolomic concentrations
  --val_condition_path VAL_CONDITION_PATH
                        validation condition in .tsv format after preprocessing, the first column name should be 'Sample' containing sample identifiers, and the rest of the columns should each denote one disease group and contain
                        either 0 or 1
  --model_name {combined_hidden}
                        name of the model architecture
  --syn_sample_count SYN_SAMPLE_COUNT
...
```
  ...
  --test_feature_path TEST_FEATURE_PATH
                        test features in .tsv format after preprocessing, the first column name should be 'Sample' containing sample identifiers and the rest of the columns should contain metabolomic concentrations
  --test_condition_path TEST_CONDITION_PATH
                        test condition in .tsv format after preprocessing, the first column name should be 'Sample' containing sample identifiers, and the rest of the columns should each denote one disease group and contain
                        either 0 or 1
  --syn_feature_path SYN_FEATURE_PATH
                        path to the synthetic data in .tsv format, the first column name should be 'Sample' containing sample identifiers and the rest of the columns should contain metabolomic concentrations
  --syn_condition_path SYN_CONDITION_PATH
                        path to the synthetic data in .tsv format, the first column name should be 'Sample' containing sample identifiers, and the rest of the columns should each denote one disease group and contain
                        either 0 or 1
  --out_dir OUT_DIR     path to output dir
```
